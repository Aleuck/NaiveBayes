...
class DigitClass:
    def __init__(self, digit):
        self.digit = digit
        self.pixels = [[] for i in range(256)]
        self.means = [-1 for i in range(256)]
        self.variances = [-1 for i in range(256)]
        self.instances = 0

def get_class_list():
    idx_file = open('indices/2.csv', 'r')
    idx_reader = csv.reader(idx_file)
    class_list = [DigitClass(i) for i in range(10)]
    ...
    for row in idx_reader:
        ...
        digit_class.instances += 1

        # add each pixel value
        for i in range(256):
            digit_class.pixels[i].append(float(usps_line[i]))

    # For each class, calculate the mean and variance for each pixel
...
for digit_class in class_list:
    print(str(digit_class.digit) + "(" + str(digit_class.instances) + "): " + str(numpy.mean(digit_class.means)))